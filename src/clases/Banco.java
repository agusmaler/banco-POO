...
import ar.edu.ort.a2019.c2.tp1.f1.tad.implementaciones.arraylist.PilaAL;
...
public class Banco {
	...
	private Cola<Cliente> filaClientesVIP;
	private Cola<Cliente> filaClientesEstandar;

	private Pila<Operacion> historialOperaciones;
	...
	public Banco(String nombre, ArrayList<Cliente> clientes, int tamanioMaximoFilas) throws RuntimeException {
		...
		this.historialOperaciones = new PilaAL<Operacion>(CANTIDAD_MAXIMA_HISTORIAL_OPERACIONES);
	 ...
	 */
	private void atender(Cola<Cliente> fila) {
		Cliente c = fila.remove();
		Operacion o = c.indicarOperacionRequerida();

		o.operar();

		this.historialOperaciones.push(o);
...
//		mostrar cantidad de operaciones por mes.
	...
	private void mostrarOperaciones() {
		Pila<Operacion> pilaAuxiliar = new PilaAL<Operacion>();
		while (!this.historialOperaciones.isEmpty()) {
			Operacion o = historialOperaciones.pop();
			o.mostrar();
			pilaAuxiliar.push(o);
		}

		while (!pilaAuxiliar.isEmpty()) {
			this.historialOperaciones.push(pilaAuxiliar.pop());
		}
